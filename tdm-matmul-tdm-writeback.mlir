// RUN: iree-compile --output-format=vm-bytecode --iree-hal-target-backends=rocm --iree-rocm-target=gfx1250 --compile-from=executable-configurations -o tdm-matmul-tdm-writeback.vmfb tdm-matmul-tdm-writeback.mlir

/// A quick prototype TDM code based on Triton.
/// Uses a 256x256x128 workgroup tile across four waves,
/// calling a 16x16x32 intrinsic. Double-buffers in LDS. Unlike the main version,
/// uses TDM to store the result too, doing manual buffer reuse.
...
!the_lds = memref<278528xi8, #gpu.address_space<workgroup>>
!tdm_store_tile = memref<256x256xf32, #gpu.address_space<workgroup>>
    ...
    hal.executable.variant public @rocm_hsaco_fb target(#gpu_target) {
      ...
      builtin.module {
        func.func @tdm_matmul_trb() attributes {translation_info = #translation_info} {
          ...
          %theLds = memref.alloc() : !the_lds
          // Ofsets of TDM tile in merged allocation.
          %cA1 = arith.constant 69632 : index
          %cB0 = arith.constant 139264 : index
          %cB1 = arith.constant 208896 : index
          %a0 = memref.view %theLds[%c0][] : !the_lds to !tdm_tile
          %a1 = memref.view %theLds[%cA1][] : !the_lds to !tdm_tile
          %b0 = memref.view %theLds[%cB0][] : !the_lds to !tdm_tile
          %b1 = memref.view %theLds[%cB1][] : !the_lds to !tdm_tile
          ...
          %c = memref.view %theLds[%c0][] : !the_lds to !tdm_store_tile
            ...
            pcf.generic scope(#iree_gpu.subgroup_scope)
              ...
              pcf.generic scope(#iree_gpu.lane_scope)
                ...
                %accLoop = scf.for %k1 = %c128 to %KL step %c256 iter_args (%acc = %accZeros) -> (vector<8x8x1x8xf32>) {
                  ...
                  // Unlike in my pure TDM example, since writeback doesn't allow a
                  // padded LDS, I'm not doing the column-major thing. I figure
                  // threads writing 16 adjacent banks and then jumping will
                  // avoid conflicts.
                  %acc0 = iree_codegen.inner_tiled ins(%lhs0, %rhs0) outs(%acc)
                    {
                  ...
                  %acc1 = iree_codegen.inner_tiled ins(%lhs1, %rhs1) outs(%acc0)
                    {
                      indexing_maps = #contraction_accesses,
                      iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>, #linalg.iterator_type<reduction>],
                      kind = #iree_gpu.mma_layout<WMMA_F32_16x16x32_F16>,
                ...
                %accFinal = iree_codegen.inner_tiled ins(%lhs_epilogue, %rhs_epilogue) outs(%accLoop)
                  {
                    indexing_maps = #contraction_accesses,
                    iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>, #linalg.iterator_type<reduction>],
                    kind = #iree_gpu.mma_layout<WMMA_F32_16x16x32_F16>,
                ...
                // Writeback
                gpu.barrier memfence [#gpu.address_space<workgroup>]
                %m_lane_store, %n_lane_store = affine.delinearize_index %laneid into (2, 16) : index, index
                scf.for %m_outer = %c0 to %c8 step %c1 {
                  scf.for %n_outer = %c0 to %c8 step %c1 {
                    scf.for %m_inner = %c0 to %c8 step %c1 {
                      %m_offset_store = affine.linearize_index disjoint [%m_outer, %m_sg, %m_lane_store, %m_inner] by (8, 2, 2, 8) : index
                      %n_offset_store = affine.linearize_index disjoint [%n_outer, %n_sg, %n_lane_store] by (8, 2, 16) : index

                      %data = vector.extract %accFinal[%m_outer, %n_outer, 0, %m_inner] : f32 from vector<8x8x1x8xf32>
                      memref.store %data, %c[%m_offset_store, %n_offset_store] : !tdm_store_tile
                    } {unroll_loop}
                  } {unroll_loop}
                } {unroll_loop}
                pcf.return
              ...
              gpu.barrier memfence [#gpu.address_space<workgroup>]
              // Stores are doing 64x256 chunks instead.
              %n_wg_off_store = arith.muli %n_wg, %c256 overflow<nsw, nuw> : index

              %n_tile_size_store_tmp = affine.min affine_map<()[s0, s1] -> (256, s0 - s1)>()[%N, %n_wg_off_store]
              %n_tile_size_store = affine.max affine_map<(d0)[] -> (d0, 0)>(%n_tile_size_store_tmp)

              %c_base = amdgpu.make_dma_base %C[%m_global_off_load, %n_wg_off_store], %c[%lds_sg_off, %c0] : !global_buffer_out, !tdm_store_tile -> !amdgpu.tdm_base<f32>
              %c_desc = amdgpu.make_dma_descriptor %c_base
                globalSize [%m_tile_size, %n_tile_size_store]
                globalStride [%N, 1]
                sharedSize [64, 256]
                : !amdgpu.tdm_base<f32> -> !amdgpu.tdm_descriptor<f32>
              amdgpu.tensor_store_from_lds %c_desc : !amdgpu.tdm_descriptor<f32>
              // Not needed at end of program, maybe?
              amdgpu.memory_counter_wait tensor(0)
              pcf.return
            }
            pcf.return
